...
export default {
    ...
    setup(props) {
        ...
        const getDetail=(id) => {
            console.log("id",id)
            axios.get(`/getHotConfig/${id}`).then(res => {
                state.ruleForm={
                    name: res.configName,
                    id: res.goodsId,
        ...
        const submitForm=() => {
            formRef.value.validate((valid) => {
                if(valid) {
                    ...
                    if(state.ruleForm.id<0||state.ruleForm.id>100000) {
                        ElMessage.error('商品编号不能小于 0 或大于 100000')
                        return
                    }
                    if(props.type=='add') {
                        axios.post('/getHotConfig/add',{
