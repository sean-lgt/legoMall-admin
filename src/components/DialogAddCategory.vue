<template>
    <el-dialog :title="type == 'add' ? '添加分类' : '修改分类'"
               v-model="visible"
               width="400px">
        <el-form :model="ruleForm"
            ...
            <el-form-item label="排序值"
                ...
                <el-input type="number"
                          v-model="ruleForm.rank"></el-input>
            </el-form-item>
        </el-form>
        <template #footer>
...
export default {
    ...
    setup(props) {
        ...
        // 获取详情
        const getDetail=(id) => {
            axios.get(`/categoriesModule/${id}`).then(res => {
                state.ruleForm={
        ...
        const submitForm=() => {
            formRef.value.validate((valid) => {
                if(valid) {
                    ...
                    if(props.type=='add') {
                        axios.post('/categoriesModule/add',{
                        ...
                        axios.put('/categoriesModule/update/'+state.id,{
                            categoryId: state.id,
                            categoryLevel: state.categoryLevel,
